...
const BooksTable: React.FC = () => {
  ...
  const updateBook = async (bookId: string) => {
    try {
      ...
      setError("Failed to update book: " + (err.response?.data?.detail?.map((e: any) => e.msg).join(", ") || "An unknown error occurred"));
    }
  };
            ...
            <tr key={index}>
              ...
              <td className={styles['table-actions']}>
                <button onClick={() => {
                  ...
                  setNewBook({
                    title: book.title,
                    authors: Array.isArray(book.authors) ? book.authors.map(author => typeof author === 'object' ? author.name : author) : [],
